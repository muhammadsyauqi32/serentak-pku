...
export default function Navbar({ onOpenRegister, onOpenGuidebook }: NavbarProps) {
  ...
  return (
    <>
      <header
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${isScrolled ? "bg-[#3D070F]/95 backdrop-blur-md py-3 shadow-xl border-b border-[#C5A059]/30" : "bg-gradient-to-b from-[#2A050A]/90 to-transparent py-5"}`}
      >
          ...
          <div className="flex items-center justify-between">
            ...
            <a href="#home" className="flex items-center gap-3 group">
              <div className="w-25 h-20 flex items-center justify-center">
                <Image src="/images/logo.png" alt="SERENTAK Logo" width={100} height={100} className="object-contain" />
              </div>
            </a>
                ...
                return (
                  <a
                    ...
                    className={`px-3 py-1.5 text-xs xl:text-sm font-medium rounded-full transition-all duration-200 ${
                      isActive ? "bg-[#8B0000] text-[#E5C378] border border-[#C5A059]/40 shadow-sm" : "text-[#FAF6F0]/80 hover:text-[#E5C378] hover:bg-[#8B0000]/30"
                    }`}
                  >
                    {link.name}
                  </a>
            ...
            <div className="hidden lg:flex items-center gap-3">
              <button onClick={onOpenGuidebook} className="flex items-center gap-1.5 px-3.5 py-2 text-xs font-semibold text-[#E5C378] border border-[#C5A059]/50 rounded-lg hover:bg-[#C5A059]/10 transition-all cursor-pointer">
            ...
            <div className="flex lg:hidden items-center gap-2">
              <button onClick={onOpenRegister} className="px-3 py-1.5 text-[11px] font-bold text-[#3D030D] bg-[#C5A059] rounded-md">
                Daftar
              </button>
              <button onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="p-2 text-[#E5C378] hover:text-white rounded-lg focus:outline-none focus:ring-2 focus:ring-[#C5A059]" aria-label="Toggle Navigation Menu">
        ...
        {mobileMenuOpen && (
          ...
          >
            <div className="flex flex-col gap-3">
              <div className="text-xs uppercase tracking-widest text-[#C5A059] font-bold pb-2 border-b border-[#C5A059]/20">Navigasi Acara</div>
              {navLinks.map((link) => (
                <a
                  key={link.name}
                  href={link.href}
                  ...
                  className={`flex items-center justify-between px-4 py-2.5 rounded-lg text-sm font-medium transition-all ${
                    activeSection === link.id ? "bg-[#8B0000] text-[#E5C378] font-bold border border-[#C5A059]/30" : "text-[#FAF6F0] hover:bg-[#8B0000]/40 hover:text-[#E5C378]"
                  }`}
                >
                  <span>{link.name}</span>
